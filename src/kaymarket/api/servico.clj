...
(defn consultar-acao-handler [codigo]
  (let [dados (client/consultar-acao-externa codigo)]
    (json-response dados)))
...
(defn- processar-venda [transacao] 
  (let [codigo (:codigo transacao)
    ...
    (if (d-venda/venda-valida? historico codigo qtd-venda)
      (do (db/registrar-transacao! transacao)
          (json-response {:status "sucesso" :mensagem "Venda registrada" :dados transacao}))
      
      (json-response {:status "erro" :mensagem (str "Saldo insuficiente para vender" qtd-venda " de " codigo)}
                     400))))
